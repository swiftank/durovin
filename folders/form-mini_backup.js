...
define([
    ...
    $.widget('smileEs.quickSearch', $.mage.quickSearch, {
        ...
        _fetchBlogPosts: function (queryString) {
            $.ajax({
                method: 'GET',
                url: '/page/ajax/request',
                cache: true,
                dataType: 'json',
                data: { q: queryString },
                beforeSend: function () {
                ...
                success: $.proxy(function (data) {
                    if( $('.custom_blog_post').length ){
                        $('.custom_blog_post').remove();
                    }
                    var newHtml = ''
                    newHtml += '<dl class="custom_blog_post">';
                    newHtml += '<dt>Blog</dt>';
                    for (var a = 0; a < data.length; a++) {
                        newHtml += '<dd><a href="/news/post/'+ data[a].identifier +'">' + data[a].title + '</a></dd>';
                    }
                    newHtml += '</dl>';
                    if(data.length > 0){
